...
fn baseline_clean_and_detached_head_use_committed_head() {
    ...
    assert_eq!(inspect_baseline(&runner(temp.path())).expect("baseline"), BaselineState::Clean { head: head.clone() });

    git(temp.path(), &["checkout", "--detach", "-q", "HEAD"]);
    assert_eq!(inspect_baseline(&runner(temp.path())).expect("detached baseline"), BaselineState::DetachedHead { head });
}
...
fn baseline_dirty_variants_require_explicit_selection_without_mutation() {
    for variant in ["staged", "unstaged", "untracked"] {
        ...
        let before = git(temp.path(), &["status", "--porcelain=v1", "--untracked-files=all"]);
        assert_eq!(inspect_baseline(&runner(temp.path())).expect("baseline"), BaselineState::SelectionRequired { code: "BASELINE_SELECTION_REQUIRED" });
        let after = git(temp.path(), &["status", "--porcelain=v1", "--untracked-files=all"]);
...
fn baseline_inspection_supports_a_linked_worktree() {
    ...
    repo(&main);
    git(&main, &["worktree", "add", "-qb", "linked-test", linked.to_str().expect("utf8 path"), "HEAD"]);
    assert!(matches!(inspect_baseline(&runner(&linked)).expect("linked baseline"), BaselineState::Clean { .. }));
}
