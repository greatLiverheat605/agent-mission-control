...
fn approval_resolution_is_revision_checked_before_dispatch() {
    ...
    let gate = actor
        ...
        .expect("gate action");
    assert_eq!(gate, DispatchGate::WaitingForApproval("approval-write".to_owned()));
    ...
    assert_eq!(
        actor.pending_approval("approval-write").map(ApprovalRequest::state),
    ...
    assert_eq!(actor.ledger().last().expect("event").kind.as_str(), "approval_consumed");
}
