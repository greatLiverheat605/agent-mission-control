...
export function fixtureMission(config: VisualFixtureConfig): MissionReadModel {
  ...
  if (!empty) events.push(
    ...
    event(missionId, events.length + 2, "loadout_snapshot", { provider: "Codex", model: "GPT-5", items: [{ name: "Playwright", status: "loaded", source: "plugin" }, { name: "Shrimp MCP", status: "loaded", source: "native" }] }),
